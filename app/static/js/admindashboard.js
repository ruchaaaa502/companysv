        ...
        .then(data => {
            ...
            if (tableBody) {
                ...
                if (data.activities && data.activities.length > 0) {
                    ...
                    tableBody.innerHTML = `<tr><td colspan="3" class="no-results">No activity data found</td></tr>`;
                }
            }
        })
...
function downloadFilteredResponses() {
    ...
    const empId = form.emp_id.value.trim(); // Not used in backend yet
...
document.getElementById("formResponsesForm").addEventListener("search", function(e) {
    e.preventDefault();
        ...
        .then(data => {
            ...
            if (tableBody) {
                ...
                if (data.responses && data.responses.length > 0) {
                    ...
                    tableBody.innerHTML = `<tr><td colspan="4" class="no-results">No responses found</td></tr>`;
                    document.getElementById("no-responses").classList.remove("hidden");
                }
            }
        })
        .catch(error => console.error("Error loading form responses:", error));
}
...
function handleCreateUser(e) {
    ...
    const bloodgrp = document.getElementById("bloodgrp").value.trim();
    const doi = document.getElementById("doi").value.trim();

    // Optional: Simple frontend validation
    if (!emp_id || !email || !name || !phoneno || !Designation || !bloodgrp || !doi) {
...
searchInput.addEventListener('input', function() {
    ...
    const filtered = featureList.filter(feature => feature.toLowerCase().includes(query));
    ...
    filtered.forEach((item, index) => {
        ...
        li.addEventListener('click', () => {
            ...
            suggestionBox.classList.add('hidden');
            searchFeatures(item);
            expandFeature(item); // ✅ Trigger expansion
        });
...
searchInput.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowDown') {
        ...
        if (selected) {
            ...
            suggestionBox.classList.add('hidden');
            searchFeatures(selected.textContent);
            expandFeature(selected.textContent); // ✅ Trigger expansion
        }
    }
});
